...
export const useKenaikanJabatanStore = defineStore("kenaikanJabatanStore", {
  state: () => ({
    responses: null,
  ...
  actions: {
    async getData(page = "") {
      ...
      try {
        const response = await axiosIns.get(
          `/kenaikan-jenjang?limit=${this.currentLimit}${this.searchQuery}${page}${this.dateQuery}`
        );
    ...
    async store() {
      ...
      try {
        const response = await axiosIns.post(`/kenaikan-jenjang`, this.form);
    ...
    async destroy(id) {
      ...
      setTimeout(() => {}, 500);
      try {
        await axiosIns.delete(`/kenaikan-jenjang/${id}`);
    ...
    async update() {
      ...
      try {
        const response = await axiosIns.put(
          `/kenaikan-jenjang/${this.singleResponses.id}`,

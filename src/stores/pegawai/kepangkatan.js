...
export const useKepangkatanStore = defineStore("kepangkatan", {
  state: () => ({
    responses: null,
  ...
  actions: {
    async getData(page = "") {
      ...
      try {
        const response = await axiosIns.get(
          `/kepangkatan?limit=${this.currentLimit}${this.searchQuery}${page}${this.dateQuery}`
        );
    ...
    async store() {
      ...
      try {
        const response = await axiosIns.post(`/kepangkatan`, this.form);
    ...
    async destroy(id) {
      ...
      setTimeout(() => {}, 500);
      try {
        await axiosIns.delete(`/kepangkatan/${id}`);
    ...
    async update() {
      ...
      try {
        const response = await axiosIns.put(
          `/kepangkatan/${this.singleResponses.id}`,
